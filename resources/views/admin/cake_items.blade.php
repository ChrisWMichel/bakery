        ...
        <div class="panel panel-default add-item-panel">
            ...
            <div class="hide-add-item">
                <div class="panel-body">
                    ...
                    {!! Form::open(['method'=>'POST', 'action'=>'CakeItemController@store', 'id' => 'item-form', 'novalidate' => 'novalidate']) !!}
                    {{ csrf_field() }}
                    <div class="form-group">

                        {!! Form::label('category_cake_id', 'Category:') !!}<br>
                        {!! Form::select('category_cake_id',  $cat_list, null, ['placeholder'=> 'Pick Category:']) !!}
                    ...
                    <div class="form-group">
                        ...
                        {!! Form::textarea('description', NULL, ['class' => 'form-control', 'id'=>'item-desc', 'rows' => 3, 'cols' => 6, 'required' => 'required']) !!}
                    ...
                    <div class="form-group">
                        {!! Form::submit('Add Item', ['class' => 'btn btn-primary pull-right add-cakeitem-frm']) !!}
                    </div>
...
@section('scripts')
    ...
    <script>
        ...
        /* Add Item Form */
        $('.add-cakeitem-frm').on('click', function (e) {
            e.preventDefault();
            $(".error").find("ul").html('');
            tinyMCE.triggerSave();
            const data = {
                'category_cake_id' : $('select[name=category_cake_id]').val(),
            ...
            const url = '{{route("cake_items.store")}}';
            $.ajax({
                url : url,
                type : 'post',
                data: data,
        ...
        $('.pages').on('click', function (e) {
            ...
            $('#show-add-items').hide();
            tinymce.remove();
            const url = "cake_items/" + id;
            $.ajax({
                url : url,
                type    : 'get',
                success : function (data) {
                    $('.display-cakeItems').html(data);
