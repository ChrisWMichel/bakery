...
@foreach($items as $item)
    ...
    <div class="panel-body tan-background" id="panel-{{$item->id}}">
        ...
        <div class="form-group">
            <div class="error{{$item->id}} text-center alert-danger hidden">
                <ul>
        ...
        <div class="row">
            ...
            <div class="col-sm-7">
                <a href="#" class="btn btn-danger btn-sm pull-right delete-item" id="{{$item->id}}">X</a>
            </div>
        </div>

        <h3 class="blue-font" id="frm-msg-{{$item->id}}" ></h3>
        {!! Form::open(['method'=>'post','action'=> ['ItemController@update', $item->id], 'id' => 'item-form', 'novalidate' => 'novalidate']) !!}
        {{ csrf_field() }}
        ...
        <div class="form-group">
            <input type="submit" class="btn btn-primary pull-right update-item-frm" id="{{$item->id}}">
        </div>
...
<script>
    ...
    /* Update item form */
    $('.update-item-frm').on('click', function (e) {
        e.preventDefault();
        tinyMCE.triggerSave();
        const id = $(this).attr('id');
        const data = {
